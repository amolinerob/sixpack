...
export function GoalProgressRing({ label, percentage, percentageLabel, status = getGoalStatus(percentage), size = 'compact' }: GoalProgressRingProps) {
  ...
  return <div className={`goal-progress-ring goal-progress-ring--${size} goal-progress-ring--${status}`} aria-label={`${label}: ${percentageLabel ?? `${displayedPercentage}%`}`}>
    ...
    <strong>{percentageLabel ?? `${displayedPercentage}%`}</strong>
  </div>
}
